class Agency < ApplicationRecord
  ...
  #   #agencies = Agency.near(params[:search])
  #   agencies = Agency.where("name LIKE ? OR address LIKE ? OR zipcode LIKE ? OR state LIKE ? OR city LIKE ? OR description LIKE ?" , "%#{params[:search]}%", "%#{params[:search]}%", "%#{params[:search]}%", "%#{params[:search]}%", "%#{params[:search]}%", "%#{params[:search]}%")
  #   agencies
  # end
  ...
  def self.search(params)
    ...
    agencies = Agency.near(params[:location], 20)
